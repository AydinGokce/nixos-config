...
impl Workbench {
    ...
    fn failure(&mut self, id: String, pending: Pending, error: rpc::RpcError) {
        ...
        self.library_structures_failed(&pending.purpose, &message);
    ...
    pub(super) fn events(&mut self, ctx: &egui::Context) {
        ...
        for event in events {
            match event {
                session::Event::Progress { id, done, total } => {
                    ...
                    if let Some(Purpose::Upload(target)) =
                        self.pending.get(&id).map(|p| p.purpose.clone())
                    {
                        self.library_structure_upload_progress(&target, done, total);
                    }
                }
        ...
        self.continue_run();
        self.library_structure_uploads_poll();
    ...
    fn received(&mut self, id: String, purpose: Purpose, value: Value, ctx: &egui::Context) {
        match purpose {
            ...
            Purpose::LibraryStructures(request) => {
                self.library_structures_received(request, value, ctx)
            }
    ...
    pub(super) fn choose_files(&self, kind: Pick, ctx: &egui::Context) {
        ...
        std::thread::spawn(move || {
            let files = match &kind {
                Pick::LibraryStructures(_) => rfd::FileDialog::new()
                    .add_filter("Protein structures", &["pdb", "cif", "mmcif"])
                    .pick_files()
                    .unwrap_or_default(),
