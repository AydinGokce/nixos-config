...
Copy `bio-workbench-rust-0.3.0-duplicate-tabs-source.tar.gz` from the existing workstation to
your Windows **Downloads** folder. The prepared archive on that workstation is:

```text
/home/aydin/bio-runs/duplicate-tabs-20260907/bio-workbench-rust-0.3.0-duplicate-tabs-source.tar.gz
```
...
tar -xzf "/mnt/c/Users/YOUR_WINDOWS_USERNAME/Downloads/bio-workbench-rust-0.3.0-duplicate-tabs-source.tar.gz" --strip-components=1 -C ~/bio-workbench-rust
cd ~/bio-workbench-rust
nix --extra-experimental-features 'nix-command flakes' build path:.
./result/bin/bio-workbench
```

This archive includes draggable viewer tabs, independent duplicate tabs, right-click Duplicate tab / Split down / Split right, persistent splits, working quick controls and the console resize fix. To update an existing installation, close the app, extract over its source directory and rebuild with the same command.
