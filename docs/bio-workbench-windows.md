...
Copy `bio-workbench-rust-0.3.0-library-explorer-source.tar.gz` from the existing workstation to
your Windows **Downloads** folder. The prepared archive on that workstation is:

```text
/home/aydin/bio-runs/library-explorer-20260909/bio-workbench-rust-0.3.0-library-explorer-source.tar.gz
```
...
tar -xzf "/mnt/c/Users/YOUR_WINDOWS_USERNAME/Downloads/bio-workbench-rust-0.3.0-library-explorer-source.tar.gz" --strip-components=1 -C ~/bio-workbench-rust
cd ~/bio-workbench-rust
nix --extra-experimental-features 'nix-command flakes' build path:.
./result/bin/bio-workbench
```

This archive includes the native **Library** explorer for project/construct navigation,
purpose documents, pinned input selection and verified original-file downloads.
