...
JOB="$TOOLS/py/evolvepro_cloud.py"
...
sys_venv "$PLM"
uv pip install --python "$PLM/bin/python" -r "$TOOLS/requirements/evolvepro-plm.txt"
if [ "${SUB:-rank}" = rank ]; then
  sys_venv "$CORE"
  uv pip install --python "$CORE/bin/python" -r "$TOOLS/requirements/evolvepro-core.txt"
fi
